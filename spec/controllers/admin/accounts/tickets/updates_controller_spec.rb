...
describe Admin::Accounts::Tickets::UpdatesController do
  it_behaves_like "a protected user controller", :update do
    let(:ticket) { create :ticket }
    let(:request_options) { { account_id: 1, ticket_id: ticket.to_param } }
  end

  it_behaves_like "a protected admin controller", :update do
    let(:ticket) { create :ticket }
    let(:request_options) { { account_id: 1, ticket_id: ticket.to_param } }
  end
